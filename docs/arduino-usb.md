...
설치 스크립트의 기본 동작도 자동 저장 비활성화로 변경했다.
자동 CSV 수집을 명시적으로 켤 때만 `--enable-logging`을 사용한다.
기존 실행 중 서비스는 소스 수정으로 중지되지 않으며 사용자 sudo 실행이 남아 있다.
[재부팅 이후 확인 결과와 통합 화면 출력 명령](../history/2026-09-13/usb-and-current-status.md)
아래 자동 수집 설치 과정은 과거 실행 이력이다.
